...
img_sample = cv2.imread(InputName, 0)
...
def OverlapErrorVertical( imgPx, samplePx ):
    ...
    for i in range( PatchSize ):
        for j in range( OverlapWidth ):
            diff =  int(img[i + iLeft, j+ jLeft]) - int(img_sample[i + iRight, j + jRight])
...
def SSD_Error( offset, imgPx, samplePx ):
    err = int(img[imgPx[0] + offset[0], imgPx[1] + offset[1]]) -int(img_sample[samplePx[0] + offset[0], samplePx[1] + offset[1]])
